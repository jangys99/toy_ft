...
user_file_name = f"User_{user_id_input}_train_dataset.json"
...
# 6. RAG 추론 함수
# -------------------------------------
def rag_inference(instruction, user_input, index, user_data, top_k=1):
    # 1. 사용자 쿼리 임베딩
    ...
    # 2. FAISS를 사용해 관련 데이터 검색 (top_k=1로 설정)
    ...
    # 4. RAG 프롬프트 템플릿: 검색된 컨텍스트를 LLM에 전달
...
if __name__ == "__main__":
    ...
    # 2. 데이터셋 순회 및 RAG 추론
    print(f"\n{user_id_input} 데이터셋의 모든 항목에 대해 RAG 추론을 시작합니다.")
    
    # 추론 결과를 저장할 리스트
    rag_inference_results = []
    
    user_test_data_path = f"User_{user_id_input}_test_dataset.json"
    user_test_data, _ = load_and_embed_user_data(user_test_data_path)
    
    # 데이터셋의 각 항목에 대해 추론을 수행합니다.
    for item in user_test_data:
        instruction = item.get("instruction", "")
        input_data = item.get("input", "")
        
        # RAG 추론 실행 (top_k=1로 설정하여 가장 유사한 1개의 컨텍스트만 사용)
        generated_response, context_used = rag_inference(
            instruction, input_data, faiss_index, user_test_data, top_k=1
        )
        
        # 결과를 딕셔너리 형태로 저장
        rag_inference_results.append({
    ...
    # 3. 결과를 JSON 파일로 저장
    output_filename = f"./rag_json/rag_inference_test_results_{user_id_input}.json"
    with open(output_filename, 'w', encoding='utf-8') as f:
        json.dump(rag_inference_results, f, indent=4, ensure_ascii=False)
